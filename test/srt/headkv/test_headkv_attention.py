"""HeadKV attention 语义测试(Phase 4.1)。
...
def _make_env(mask_layer):
    ...
    # 单测环境未走 server 初始化,手动设置 attention TP 全局状态
    ...
    allocator = HeadReallocAllocator(
        1024, 4 * V, torch.float32, "cpu", pool, need_sort=False, window_size=V,
    )
...
def _decode(batch, pool, allocator, backend, req_idx, seq_len):
    ...
    # 本测试约定:调用时 seq_lens = 新长度(含新 token),新 token 位置 = seq_len - 1
    class FB:
        batch_size = 1
        req_pool_indices = torch.tensor([req_idx])
        seq_lens = torch.tensor([seq_len])
...
class TestExtendMapping:
    ...
    def test_extend_short_seq_all_mapped(self):
        ...
        assert mapping[9].item() == comp_base + 9  # 都在 sink 区内
    ...
    def test_chunked_extend_recent_moves(self):
        ...
        assert (mapping[14:] > 0).all()  # 30..59 中 44..59 → mapping[14..29]
        assert (mapping[:14] == 0).all()  # 30..43 → 0
...
class TestDecodeMapping:
    def test_decode_ring_wraparound(self):
        ...
        req_to_token = backend.req_to_token
        # 从位置 30 开始 decode 到 40(seq 已 > sink)
    ...
    def test_ring_overwrites_old_recent(self):
        ...
        req_to_token = backend.req_to_token
        # decode 位置 30:槽 = comp_base + sink + (30-8)%16 = +6
        ...
        # 再 decode 16 步:位置 46 → (46-8)%16 = 6 → 同一槽(覆盖)
...
class TestRefAttention:
    ...
    def test_ref_matches_numpy_manual(self):
        ...
        # 手算:T=1 的 causal 全可见 → 全 L 个 KV 的 attention
        s = q @ k.transpose(-2, -1) / (8 ** 0.5)  # [1, L]
        attn = torch.softmax(s, dim=-1)
        expected = attn @ v  # [1, 1, dim]
